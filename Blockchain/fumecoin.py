...
def mine_block():
    ...
    blockchain.add_transaction(sender=node_address, receiver='Yago', amount=1)
...
def is_valid():
    ...
    if is_valid:
        ...
        response = {'message':'Hay un problema. El blockchain no es válido'}
    return jsonify(response), 200

# Agragando nueva transaccion al blockchain
...
def connect_node():
    ...
    response = {'message':'Todos los nodos están ahora conectados. El FumeCoin Blockchain contiene los siguientes nodos:',
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message':'Los nodos tenían diferentes cadenas, así que la cadena fue reemplazada por la más larga',
